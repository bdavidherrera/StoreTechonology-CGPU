...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 02-09-2025 a las 20:39:08
...
--
-- Base de datos: `ecommerce-storetecnology`
--
...
INSERT INTO `empleados` (`id_empleados`, `cedula`, `nombres`, `apellidos`, `email`, `telefono`, `direccion`, `salario_base`, `estado`, `fecha_creacion`) VALUES
(1, '12345678', 'Juan', 'Pérez', 'juan@empresa.com', NULL, NULL, 1300000.00, 'activo', '2025-09-02 18:25:02');
...
(16, 1, 'salida', 2, 'producto dañado en transporte', '2025-09-02 18:36:25'),
(17, 54, 'salida', 5, 'muestras para exhibición', '2025-09-02 18:36:25');
...
CREATE TABLE `producto` (
  ...
  `stock_minimo` int(11) DEFAULT 10 COMMENT 'Stock mínimo para alertas'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
(53, 'Computador Portátil Gamer ASUS TUF F 16\" Pulgadas FX607VJ', 'Portátil Lenovo IdeaPad 15AMN Ryzen 5 7520U RAM 8GB 256GB SSD 15,6.jpg', 12112.00, 150, 'asSAASASAS', '2025-08-26 19:21:50', 1, 19.00, 8000.00, 3),
...
CREATE TABLE `retenciones` (
  ...
  `monto_minimo` decimal(12,2) NOT NULL COMMENT 'Monto mínimo para aplicar retención',
...
(1, 'empresa', 1000000.00, 2.50, 'Retención en la fuente - Compras generales', 1),
(2, 'empresa', 5000000.00, 3.50, 'Retención en la fuente - Compras mayores', 1),
(3, 'persona', 2000000.00, 1.00, 'Retención en la fuente - Personas naturales', 1),
(4, 'empresa', 800000.00, 2.00, 'Retención comercio electrónico', 1),
(5, 'persona', 3000000.00, 1.50, 'Retención personas naturales - montos altos', 1);
...
CREATE TABLE `sueldo` (
  ...
  `transporte` decimal(10,2) DEFAULT 140000.00,
  `prima` decimal(10,2) DEFAULT 0.00,
  `vacaciones` decimal(10,2) DEFAULT 0.00,
  `descuento_salud` decimal(10,2) GENERATED ALWAYS AS (`valor_base` * 0.04) VIRTUAL,
  `descuento_pension` decimal(10,2) GENERATED ALWAYS AS (`valor_base` * 0.04) VIRTUAL,
  `total_pagar` decimal(10,2) GENERATED ALWAYS AS (`valor_base` + `transporte` + `prima` + `vacaciones` - `valor_base` * 0.08) VIRTUAL,
  `periodo` date NOT NULL COMMENT 'Mes del sueldo (YYYY-MM-01)',
...
CREATE TABLE `ventas` (
  ...
  `valor_retencion_porcentaje` decimal(5,2) DEFAULT 0.00 COMMENT 'Porcentaje de retención aplicado',
  `monto_recibido` decimal(10,2) GENERATED ALWAYS AS (`monto_total` - `retencion_fuente`) VIRTUAL COMMENT 'Dinero realmente recibido',
  `aplica_retencion` tinyint(1) DEFAULT 0 COMMENT '1 si el cliente aplica retención'
...
--
-- Estructura Stand-in para la vista `v_dashboard_mes`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `v_dashboard_mes` (
`ventas_mes` decimal(32,2)
,`utilidad_bruta` decimal(43,2)
...
--
-- Estructura para la vista `v_dashboard_mes`
--
DROP TABLE IF EXISTS `v_dashboard_mes`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_dashboard_mes`  AS SELECT coalesce(sum(`v`.`monto_total`),0) AS `ventas_mes`, coalesce(sum(`v`.`monto_total` - `dp`.`cantidad` * `p`.`precio_costo`),0) AS `utilidad_bruta`, count(`v`.`idVenta`) AS `total_ventas` FROM (((`ventas` `v` left join `pedidos` `pe` on(`v`.`idPedido` = `pe`.`idPedido`)) left join `detallepedido` `dp` on(`pe`.`idPedido` = `dp`.`idPedido`)) left join `producto` `p` on(`dp`.`idProducto` = `p`.`idProducto`)) WHERE month(`v`.`fecha_venta`) = month(current_timestamp()) AND year(`v`.`fecha_venta`) = year(current_timestamp()) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_inventario`  AS SELECT `p`.`idProducto` AS `idProducto`, `p`.`nombreProducto` AS `nombreProducto`, `p`.`cantidad` AS `stock`, `p`.`precio_costo` AS `precio_costo`, `p`.`valor` AS `precio_venta`, CASE WHEN `p`.`cantidad` <= `p`.`stock_minimo` THEN 'BAJO' ELSE 'OK' END AS `estado` FROM `producto` AS `p` WHERE `p`.`activo` = 1 ;
...
ALTER TABLE `sueldo`
  ...
  ADD UNIQUE KEY `uk_sueldo_empleado_periodo` (`id_empleados`,`periodo`);
...
--
ALTER TABLE `sueldo`
  ADD CONSTRAINT `sueldo_ibfk_1` FOREIGN KEY (`id_empleados`) REFERENCES `empleados` (`id_empleados`) ON DELETE CASCADE;
